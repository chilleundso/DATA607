...
kable(head(confsGlobal[,1:10]))
kable(head(confsUS[,1:10]))
```
...
confsGloballong <- tidyr::gather(confsGlobal, "Date", "Amount", -Province.State, -Country.Region, -Lat, -Long)
#transforming the date column into a date structure:
confsGloballong$Date<-as.Date(confsGloballong$Date,format='X%m.%d.%y')
...
# indices table
I am putting this at the bottom for now until we figure out how to clean up this RMD. 
```{r}
library(jsonlite)
library(tidyverse)
